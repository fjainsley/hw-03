...
gss16_advfront_aug <- augment(gss16_advfront_fit$fit)

ggplot(gss16_advfront_aug, mapping = aes(x = .fitted, y = .resid)) +
  geom_point(alpha = 0.5) +
  geom_hline(yintercept = 0, color = "red") +
  labs(x = "Predicted emailhr", y = "Residuals")
...
```{r}
 gss16_rec_1 <- recipe(advfront ~ educ, data = gss16_advfront) %>%
...
```{r}
gss16_rec_2 <- recipe(advfront ~ ., data = gss16_advfront) %>%
step_rm(emailhr) %>%
step_dummy(all_nominal(), -all_outcomes()) %>%
step_zv(all_predictors())
...
gss16_pred_2 %>%
    mutate(
        advfront      = if_else(advfront == "true", "Agree", "Disagree"),
        pred_Agree  = if_else(.pred_Agree > cutoff_prob, "Predicted Agree", "Predicted Disagree")
    ) %>%
    count(pred_Agree , advfront) %>%
    pivot_wider(names_from = advfront, values_from = n) %>%
    knitr::kable(col.names = c("Prediction", "Number"))
```
